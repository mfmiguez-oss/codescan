...
const children = [
  ...
  P("The feedback prior adapts scores; the calibration report measures whether the scores were right to begin with. It joins each manual confirmed / false_positive decision to the machine-belief snapshot frozen with it (5.7) and reports: the confirm rate by predicted-score bucket (0-39 / 40-59 / 60-79 / 80-100) — rising with the bucket means the composite score predicts analyst outcomes, flat or inverted means the weights or the AI stage need work; the score separation (mean predicted score of confirmed vs false-positive decisions); and the noisiest keys — CWE families/components whose manual history is dominated by false positives (the same keys the feedback prior nudges), a concrete scanner-tuning worklist. Like the prior, it uses only accuracy states, and it is read-only — it changes nothing, it grades. Decisions persisted before snapshots existed count toward totals but cannot be bucketed (reported as unscored). Surfaced as GET /api/calibration, the UI Calibration tab, and codescan calibration on the CLI. Because the snapshot captures what the configured pipeline predicted, the report also makes provider/model routing changes (5.5) empirically comparable over time."),
  ...
  ...bullets([
    ...
    "State store (tests/test_validation.py) — atomic save round-trip, no temp leftover, crash-during-replace preserves the existing file, machine-belief snapshot capture, analyst-note round-trip, legacy (pre-snapshot) entries loading cleanly.",
    ...
    "Calibration (tests/test_calibration.py) — bucket/rate math, accuracy states only, noisy-key surfacing, legacy decisions counted but unbucketed, and the empty-store zeroed report; endpoint coverage in test_web.py.",
    ...
    "Feedback loop (tests/test_feedback.py) — false-positive history lowers / confirmed raises the score, min-evidence gate, self-exclusion, KEV-floor respect, disabled no-op, accuracy-states-only, store attribute round-trip, and TriageHistory prompt-context counts and analyst notes (labeled, capped).",
  ...
  ...bullets([
    ...
    "feedback — analyst-feedback loop: enabled, max_adjust, min_evidence (score prior), prompt_history (triage history into the AI prompt).",
