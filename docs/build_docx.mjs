// DOCX generation script (disabled) — previously built a Word edition of the
// design document with the embedded architecture diagram. The script is now a no-op.
...
const children = [
  ...
  table(
    ...
    [
      ...
      ["AI tooling", "llm.py — task-to-model router over the Microsoft Foundry provider."],
    ],
    [3000, 6360]),
  ...
  table(
    ...
    [2000, 3200, 4160]),
  P("The provider omits effort / adaptive thinking for models that don't support them (Haiku), retries with the schema prompted instead of enforced when a Foundry workspace rejects structured outputs (beta there, per model/workspace), and registers the Anthropic SDK's client-side refusal fallback for Fable/Mythos (security tooling can trip false-positive classifier refusals; the request transparently re-serves on Opus 4.8 — Foundry has no server-side fallback support). Config ai.tasks.<name> overrides any field per task. Adding a new AI stage is a one-liner."),
  P("Enterprise / Fable 5. For security triage, capability on the judgement tasks converts to outcomes, so the enterprise profile (config/config.enterprise.yaml) routes exploitability / threat modeling / whitebox review to Fable 5 and keeps mechanical work on Haiku. codescan handles Fable's enterprise behaviors: the refusal fallback above, with data residency and retention governed by the Azure region and policy of the Foundry resource; a rejected Fable request surfaces with actionable guidance (route to Opus 4.8)."),
  P("Silent adaptive routing (ai.auto_route, off by default). When enabled, each AI call is nudged up or down the Claude capability ladder — Haiku -> Sonnet -> Opus -> Fable — relative to its configured tier, by a difficulty signal the calling stage computes (group_difficulty / size_difficulty): a single low-severity finding downgrades (cheaper), while a KEV / multi-critical / large group upgrades (stronger). It only shifts Claude ladder models — custom deployment names and other model families are untouched — and clamps at both ends. Enabling it is the operator's explicit opt-in; thereafter it applies silently per call."),
  P("Fan-out (complete_json_many). The judgement-heavy stages each build one request per repo/service and hand the list to LLMClient.complete_json_many, which owns the fan-out and returns {custom_id: result}: bounded concurrency (ai.max_concurrency, default 4) via resilient_map — up to N at once, per-item failures isolated, deterministic apply order, latency-only (same requests/cost). Prompt caching is deliberately omitted: the static system prompts sit below the model's minimum cacheable-prefix size and payloads differ."),
  ...
  ...bullets([
    ...
    "Refusal handling — on Fable/Mythos the client registers client-side fallbacks so a false-positive classifier refusal is transparently re-served; a genuine refusal surfaces as an error and deterministic scoring still stands.",
    ...
    "Resource controls against unbounded consumption (ratelimit.py, OWASP LLM04/10) — a per-client token-bucket limiter throttles /api/* (server.rate_limit_rpm/burst, keyed by SSO actor or client IP) ahead of the token guard, so a flood is cheap to reject with 429; a per-scan ceiling (server.max_findings_per_scan) bounds the cost of any single run, keeping the highest-severity findings and recording a scan.truncated audit event when it trims. Set at startup; behind a load balancer the proxy's limiter is still preferred.",
  ...
  ...bullets([
    ...
    "Horizontal path: the per-service AI calls are embarrassingly parallel — run concurrently (ai.max_concurrency) to cut wall-clock time.",
  ]),
  ...
  ...bullets([
    ...
    "AI cost/latency — per-service calls run with bounded parallelism (ai.max_concurrency) via complete_json_many.",
  ...
  ...bullets([
    ...
    "Enterprise / Fable 5 (tests/test_enterprise.py) — the enterprise config profile routing deep tasks to Fable / mechanical to Haiku, with HA storage and SIEM audit wired.",
    ...
    "Web API (tests/test_web.py) — FastAPI TestClient over state, scan, state-change (persisted across rescan), validation, ServiceNow, the API-token guard, and rate limiting (429 on flood, healthz unthrottled, limiter wrapping auth so bad-token brute-force is throttled).",
    "Rate limiter (tests/test_ratelimit.py) — token-bucket burst/refill, per-client isolation, idle eviction.",
    "CLI wiring (tests/test_cli.py) — --whitebox enables the built-in OpenHack engine for the target repo and is rejected with --no-ai.",
    "Live CLI smoke (tests/test_cli_live.py) — opt-in, excluded from the offline gate (skips unless CODESCAN_LIVE_TEST=1): loads .env, picks a Claude deployment that exists on the Foundry resource, and runs the real codescan scan CLI over the fixtures with the AI stages live — proving the CLI -> pipeline -> Foundry -> ServiceNow-export path end to end.",
  ...
  ...bullets([
    "ai — default tier + per-task routing (tasks.<name>), plus max_concurrency (bounded parallelism) and auto_route (silent adaptive tier selection). See config/config.enterprise.yaml for a Fable-5 enterprise profile.",
    "source / bitbucket / github — repo inventory (scan surface), tokens, scoping, TLS.",
    "snyk / xray — findings endpoints, tokens, TLS.",
    "openhack — whitebox review: enabled/findings_dir (ingest), auto/clone/command (run), and built-in-engine tuning (passes, pass_models for per-pass models, max_files, max_file_bytes, min_confidence).",
...
// DOCX generation disabled: the Word edition is no longer maintained.
// If needed, reintroduce generation here. For now the script is a no-op.
console.log("DOCX generation disabled; this script is a no-op now.");
