...
const children = [
  ...
  ...bullets([
    ...
    "State store (tests/test_validation.py) — atomic save round-trip, no temp leftover, crash-during-replace preserves the existing file, machine-belief snapshot capture, legacy (pre-snapshot) entries loading cleanly.",
    ...
    "Feedback loop (tests/test_feedback.py) — false-positive history lowers / confirmed raises the score, min-evidence gate, self-exclusion, KEV-floor respect, disabled no-op, accuracy-states-only, store attribute round-trip, and TriageHistory prompt-context counts.",
